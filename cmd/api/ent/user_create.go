...
func (uc *UserCreate) SetStar(i int) *UserCreate {
	uc.mutation.SetStar(i)
	return uc
}
...
func (uc *UserCreate) AddOtp(u ...*UserOtp) *UserCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (uc *UserCreate) AddQrcodeIDs(ids ...int) *UserCreate {
	uc.mutation.AddQrcodeIDs(ids...)
	return uc
}

// AddQrcode adds the "qrcode" edges to the Qrcode entity.
func (uc *UserCreate) AddQrcode(q ...*Qrcode) *UserCreate {
	ids := make([]int, len(q))
	for i := range q {
		ids[i] = q[i].ID
	}
	return uc.AddQrcodeIDs(ids...)
}
...
func (uc *UserCreate) check() error {
	...
	if _, ok := uc.mutation.Star(); !ok {
		return &ValidationError{Name: "star", err: errors.New(`ent: missing required field "User.star"`)}
	}
	return nil
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.Star(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeInt,
			Value:  value,
	...
	if nodes := uc.mutation.QrcodeIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
