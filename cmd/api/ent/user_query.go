...
func (uq *UserQuery) QueryOtp() *UserOtpQuery {
	query := &UserOtpQuery{config: uq.config}
...
// QueryQrcode chains the current query on the "qrcode" edge.
func (uq *UserQuery) QueryQrcode() *QrcodeQuery {
	query := &QrcodeQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withQrcode: uq.withQrcode.Clone(),
...
func (uq *UserQuery) WithOtp(opts ...func(*UserOtpQuery)) *UserQuery {
	query := &UserOtpQuery{config: uq.config}
...
// the "qrcode" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithQrcode(opts ...func(*QrcodeQuery)) *UserQuery {
	query := &QrcodeQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOtp; query != nil {
		if err := uq.loadOtp(ctx, query, nodes,
			func(n *User) { n.Edges.Otp = []*UserOtp{} },
			func(n *User, e *UserOtp) { n.Edges.Otp = append(n.Edges.Otp, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withQrcode; query != nil {
		if err := uq.loadQrcode(ctx, query, nodes,
			func(n *User) { n.Edges.Qrcode = []*Qrcode{} },
			func(n *User, e *Qrcode) { n.Edges.Qrcode = append(n.Edges.Qrcode, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadOtp(ctx context.Context, query *UserOtpQuery, nodes []*User, init func(*User), assign func(*User, *UserOtp)) error {
	...
	query.Where(predicate.UserOtp(func(s *sql.Selector) {
		s.Where(sql.InValues(user.OtpColumn, fks...))
	}))
...
func (uq *UserQuery) loadQrcode(ctx context.Context, query *QrcodeQuery, nodes []*User, init func(*User), assign func(*User, *Qrcode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Qrcode(func(s *sql.Selector) {
		s.Where(sql.InValues(user.QrcodeColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
